...
export default function SymptomTextInputScreen() {
    ...
    // ✅ 버튼 클릭 시 텍스트와 모드를 저장하고 predicting-text로 이동
    const handleTextPrediction = async (mode: "llm" | "nlp") => {
        if (!text.trim()) {
            Alert.alert("⚠️ 증상을 입력해 주세요.");
            return;
        }
        await AsyncStorage.setItem("symptomText", text);
        await AsyncStorage.setItem("predictMode", mode);
        router.push("/(record)/predicting-text");
    };
    ...
    return (
        ...
        >
            ...
            <View style={styles.body}>
                ...
                >
                    <TouchableOpacity
                        onPress={() => handleTextPrediction("nlp")}
                    ...
                    <TouchableOpacity
                        onPress={() => handleTextPrediction("llm")}
                        style={styles.button}
                    >
